...
`, v, "\r\n"));
  ...
  function readConfig() {
    const configFile = (0, import_fs.readFileSync)(path.resolve(__dirname, "../config.local.json"), { encoding: "utf8" });
  ...
  // package.json
  var name = "overlaycache";
  var version = "1.0.0";

  // node_modules/node-fetch/src/index.js
  var import_node_http2 = __toESM(__require("node:http"), 1);
  var import_node_https = __toESM(__require("node:https"), 1);
  var import_node_zlib = __toESM(__require("node:zlib"), 1);
  var import_node_stream2 = __toESM(__require("node:stream"), 1);
  ...
  var import_node_stream = __toESM(__require("node:stream"), 1);
  ...
  var import_node_util2 = __require("node:util");
  var import_node_http = __toESM(__require("node:http"), 1);
  ...
  var import_node_util3 = __require("node:util");
  ...
  var import_node_net = __require("node:net");
  ...
  function fetchWithTimeout(_0) {
    return __async(this, arguments, function* (resource, options = {}) {
      const { timeout = 4e3 } = options;
      const controller = new AbortController();
      const id = setTimeout(() => controller.abort(), timeout);
      const response = yield fetch(resource, __spreadProps(__spreadValues({}, options), {
  ...
  // src/helper/RequestHelper.ts
  function initRequestSchedule() {
    const config = getConfig();
    setInterval(() => __async(this, null, function* () {
      yield requestData("twitch", `${config.web.url}/?getdata=twitch`);
      yield requestData("announcement", `${config.web.url}/?variable=announcement`);
      yield requestData("ads", `${config.web.url}/?getad=true&token=${config.web.token}`);
      yield requestData("theme", `${config.web.url}/?getthemecolor`);
    }), 1e3 * 5);
  }
  function requestData(key, url) {
    return __async(this, null, function* () {
      try {
        const request = yield fetchWithTimeout(url);
